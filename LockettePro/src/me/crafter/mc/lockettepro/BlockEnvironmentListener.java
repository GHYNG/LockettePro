...
public class BlockEnvironmentListener implements Listener {
	...
	public void onVillagerOpenDoor(EntityInteractEvent event) {
		...
		if(event.getEntity() instanceof Villager && (LocketteProAPI.isSingleDoorBlock(event.getBlock()) || LocketteProAPI.isDoubleDoorBlock(event.getBlock())) && LocketteProAPI.isProtected(event.getBlock())) {
			event.setCancelled(true);
		}
	}
